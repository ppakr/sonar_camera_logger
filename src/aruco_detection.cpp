...
public:
  ArucoDistanceNode() : Node("aruco_distance_node") {
    ...
    if (!fs_calib.isOpened()) {
      RCLCPP_FATAL(get_logger(), "Cannot open calibration file: %s",
                   calib_file.c_str());
    ...
    fs_calib.release();
    RCLCPP_INFO(get_logger(), "Calibration loaded from: %s",
                calib_file.c_str());
    ...
    // ────────────────────────────────────────────────────────
    pose_pub_ =
        create_publisher<geometry_msgs::msg::PoseArray>("/aruco/poses", 10);
    ids_pub_ =
        create_publisher<std_msgs::msg::Int32MultiArray>("/aruco/ids", 10);
    distance_pub_ = create_publisher<std_msgs::msg::Float64MultiArray>(
        "/aruco/distances", 10);
    debug_pub_ =
        create_publisher<sensor_msgs::msg::Image>("/aruco/image_debug", 10);
    img_pub_ =
        create_publisher<sensor_msgs::msg::Image>("/aruco/image_raw", 10);
    ...
    timer_ = create_wall_timer(
        period, std::bind(&ArucoDistanceNode::timerCallback, this));

    RCLCPP_INFO(get_logger(),
                "ArUco distance node ready (camera=%d, DICT_4X4_50, "
                "marker_size=%.3f m)",
...
private:
  void timerCallback() {
    ...
    // ── Publish raw image (no overlay)
    // ───────────────────────────────────────
    std_msgs::msg::Header raw_hdr;
    raw_hdr.stamp = stamp;
    raw_hdr.frame_id = camera_frame_;
    img_pub_->publish(*cv_bridge::CvImage(raw_hdr, "bgr8", frame).toImageMsg());
    ...
    for (size_t i = 0; i < N; ++i) {
      cv::solvePnP(obj_pts, corners[i], camera_matrix_, dist_coeffs_, rvecs[i],
                   tvecs[i]);
      cv::drawFrameAxes(frame, camera_matrix_, dist_coeffs_, rvecs[i], tvecs[i],
                        half * 0.5f, 2);
      ...
      cv::projectPoints(ctr_3d, rvecs[i], tvecs[i], camera_matrix_,
                        dist_coeffs_, ctr_proj);
      ...
      cv::putText(frame, "ID " + std::to_string(ids[i]), cv::Point(tx, ty),
                  cv::FONT_HERSHEY_SIMPLEX, fs + 0.05, col, 2);
      ty += 18;
      cv::putText(frame,
                  cv::format("x:%.3f y:%.3f z:%.3f m", tvecs[i][0], tvecs[i][1],
                             tvecs[i][2]),
                  cv::Point(tx, ty), cv::FONT_HERSHEY_SIMPLEX, fs, col, 1);
      ty += 18;
      cv::putText(
          frame, cv::format("R:%.1f P:%.1f Y:%.1f deg", rpy[0], rpy[1], rpy[2]),
          cv::Point(tx, ty), cv::FONT_HERSHEY_SIMPLEX, fs, col, 1);
    }
    ...
    for (size_t i = 0; i < N; ++i) {
      ...
      cv::projectPoints(centre_3d, rvecs[i], tvecs[i], camera_matrix_,
                        dist_coeffs_, proj);
      centres_2d[i] = proj[0];
    }
    ...
    // N×N symmetric distance matrix; data[i*N + j] = metres between marker i
    // and j
  ...
  // 3×3 rotation matrix -> [roll, pitch, yaw] in degrees (ZYX / extrinsic XYZ
  // convention)
  ...
  static std::array<double, 4> rotMatToQuat(const cv::Mat &R) {
    const double trace =
        R.at<double>(0, 0) + R.at<double>(1, 1) + R.at<double>(2, 2);
    double w = std::sqrt(std::max(0.0, 1.0 + trace)) / 2.0;
    double x =
        std::sqrt(std::max(0.0, 1.0 + R.at<double>(0, 0) - R.at<double>(1, 1) -
                                    R.at<double>(2, 2))) /
        2.0;
    double y =
        std::sqrt(std::max(0.0, 1.0 - R.at<double>(0, 0) + R.at<double>(1, 1) -
